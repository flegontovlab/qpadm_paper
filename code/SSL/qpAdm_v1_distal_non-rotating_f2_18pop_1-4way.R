...
###randomized non-rotating distal qpAdm###
...
proxies <- grep("_100", poplist, value=T)
...
for (t in selected_targets){
  iters <- c(1:2)
  for (i in iters){
    selected_right <- sample(right, 15, replace=F) %>% sort()
    selected_proxies <- sample(proxies, 15, replace=F) %>% sort()
    ...
    extract_f2(pref=paste0("./stepping-stone_simulations_4way/",args[1]), outdir=paste0(args[1],"_distal_non-rotating_18pop_",t,"_",i), 
               pops=c(t, selected_proxies, selected_right), auto_only=F, maxmiss=0, minac2=F, 
               blgsize=4000000, adjust_pseudohaploid=F, maxmem = 50000, overwrite=T, verbose=T)
    f2_blocks = f2_from_precomp(paste0(args[1],"_distal_non-rotating_18pop_",t,"_",i))
    ...
    write.table(outtab, file=paste0(args[1],"_distal_non-rotating_18pop_",t,"_",i,"_1way.txt"), sep='\t', row.names=F)
    ...
    write.table(outtab, file=paste0(args[1],"_distal_non-rotating_18pop_",t,"_",i,"_2way.txt"), sep='\t', row.names=F)
    ...
    write.table(outtab, file=paste0(args[1],"_distal_non-rotating_18pop_",t,"_",i,"_3way.txt"), sep='\t', row.names=F)
    ...
    write.table(outtab, file=paste0(args[1],"_distal_non-rotating_18pop_",t,"_",i,"_4way.txt"), sep='\t', row.names=F)
  }
}
