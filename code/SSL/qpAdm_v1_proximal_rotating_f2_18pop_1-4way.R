...
iters <- c(1:40)
  for (i in iters){
  try({
    selected_pops <- sample(poplist, 19, replace=F) %>% sort()
    targets <- c(grep("_present", selected_pops, value=T), grep("_100", selected_pops, value=T))
    t <- sample(targets, 1, replace=F)
    selected_rotating <- selected_pops[selected_pops != t]
    ...
    extract_f2(pref=paste0("./stepping-stone_simulations_4way/",args[1]), outdir=paste0(args[1],"_proximal_rotating_18pop_",t,"_",i), 
    ...
    f2_blocks = f2_from_precomp(paste0(args[1],"_proximal_rotating_18pop_",t,"_",i))
    ...
    write.table(outtab, file=paste0(args[1],"_proximal_rotating_18pop_",t,"_",i,"_1way.txt"), sep='\t', row.names=F)
    ...
    write.table(outtab, file=paste0(args[1],"_proximal_rotating_18pop_",t,"_",i,"_2way.txt"), sep='\t', row.names=F)
    ...
    write.table(outtab, file=paste0(args[1],"_proximal_rotating_18pop_",t,"_",i,"_3way.txt"), sep='\t', row.names=F)
    ...
    write.table(outtab, file=paste0(args[1],"_proximal_rotating_18pop_",t,"_",i,"_4way.txt"), sep='\t', row.names=F)
  })
}
